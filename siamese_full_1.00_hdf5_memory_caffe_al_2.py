...
# generate num_al samples and add the k best samples to training set for next epoch
# num_al should be increased as long as the num of epoch grows up
# k could be a static number in order to keep the training set steadily grows
num_al = 5000
k = 1000
threshold = 20.5
...
for epoch in range(num_epochs):
    ...
    if epoch < num_epochs - 1: # active learning in last epoch is unessesary
        print("["+str(datetime.datetime.now().time())+"]  Active learning step: number of search samples = {}, updating data...".format(num_al))
        if len(al_idx) > num_al: # test num_al samples in al dataset
        ...
        k_best_idx = np.empty(0, dtype=np.int64)
        margin_vec = np.empty(0)
        for batch in iterate_minibatches(X_train[al_idx[pos:pos+num_al].tolist()], X_train_p[al_idx[pos:pos+num_al].tolist()], y_train[al_idx[pos:pos+num_al].tolist()], test_batch_size): # don't shuffle the al data
            ...
            solver.test_nets[0].set_input_arrays(np.concatenate((X, X_p), axis=0), np.concatenate((y, y), axis=0).astype(np.float32))
            solver.test_nets[0].forward()
            margin_vec = np.concatenate((margin_vec, np.abs(solver.test_nets[0].blobs['l2_norm'].data - threshold)), axis=0) # update margin vec
        idx_interval = np.arange(pos, pos+num_al) # the index set of idx_al in this whole al test
        #print(margin_vec[np.sort(np.argsort(margin_vec)[:20])])
        k_best_idx = idx_interval[np.sort(np.argsort(margin_vec)[:k])] # retrieve the k best samples
        train_idx = np.concatenate((train_idx, al_idx[k_best_idx]), axis=0) # add selected samples to train dataset
        train_idx.sort()
        al_idx = np.delete(al_idx, k_best_idx) # remove selected samples from al dataset
        num_al += 1000 # increase the number of al test samples
